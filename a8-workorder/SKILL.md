...
| 登录账号 | `<A8_USERNAME>` |
| 登录密码 | `<A8_PASSWORD>` |
| 浏览器 profile | `playwright` |
| 腾讯文档权限策略 | `<TENCENT_DOC_POLICY>` |

> **⚠️ 安全提示**：
> - 公开仓库必须使用占位符
> - 本地环境在 `~/.hermes/config.yaml` 和 `~/.hermes/.env` 中配置真实值
> - `.gitignore` 应包含 `.env` 文件，防止敏感信息泄露

### 配置说明

| 参数 | 说明 | 本地位置 |
|------|------|---------|
| `base_url` | A8 系统基础 URL | `config.yaml: skills.a8_workorder.base_url` |
| `login_url` | 登录页面 URL | `config.yaml: skills.a8_workorder.login_url` |
| `username` | 登录账号 | `config.yaml` 或 `.env: A8_USERNAME` |
| `password` | 登录密码 | `config.yaml` 或 `.env: A8_PASSWORD` |
| `doc_policy` | 腾讯文档权限策略 | `config.yaml: skills.a8_workorder.doc_policy` |
| `browser_profile` | 浏览器 profile | `config.yaml: skills.a8_workorder.browser_profile` |
