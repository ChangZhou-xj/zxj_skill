...
> **⚠️ 已废弃**：本文档已过时，请使用 **[playwright-mcp.md](./playwright-mcp.md)**。
>
> 旧版 `browser()` 函数已被 Playwright MCP 工具替代。新工具提供更完整的功能和更好的性能。

## 核心原则

- 始终使用**隔离浏览器 profile**，不要在用户日常使用的浏览器会话中执行自动化。
- 公开仓库中的 profile 名称使用占位符或示例值；若你的团队约定使用 `playwright`，可以在私有配置中落地。
